...
tutorial_text = """
You need to enter a valid username, choose a difficulty, and then answer each question.\n\n\
You will then be shown a results screen with your provided answers if incorrect.\n\n\
...
def main():
    ...
    def tutorial():
        messagebox.showerror("Help", tutorial_text)

    def alt_questions():
        filename = filedialog.askopenfilename(title="Select JSON file", initialdir="/", filetypes=[("Json", '*.json')])
        try:
            gather_data(filename)
            global source_file
            messagebox.showinfo("Success", "Worked perfectly, changed may be buggy unless resarted")
            if __debug__:
            ...
            if __debug__:
                print("Failed")
                messagebox.showerror("Error with custom file", "Your file apears to be an incompatible format, Please check and try again.")
            pass

    menubar = tk.Menu(root)
    root.config(menu=menubar)
    filemenu = tk.Menu(menubar, tearoff=0)
    filemenu.add_command(label="Create Custom Questions", command=json_maker.create_file)
    filemenu.add_command(label="Select Custom Level set", command=alt_questions)
    ...
    def level(hardness, index):
        ...
        def show_results():
            ...
            lable = ttk.Label(div, text=f"Excellent Job, {username}! \n You've completed the quiz!")
            lable.pack()
            for i, questions in enumerate(question_set):  # question_set[i]
                try:
                    lable = ttk.Label(div, text=f"Question: {question_set[i][0]} Correct answer was: {question_set[i][1]} (You entered: {question_set[i][2]})").pack()
                    incorrect_answers += 1
                except IndexError:
                    lable = ttk.Label(div, text=f"Question: {question_set[i][0]} Correct answer was: {question_set[i][1]}").pack()
            total_answers, correct_answers = len(question_set), len(question_set) - incorrect_answers
            lable = ttk.Label(div, text=f"You got {correct_answers} right of {total_answers}. This means you got a score of {int((correct_answers/total_answers) * 100)}%").pack()
            button = ttk.Button(div, text="Start Again?", command=lambda: [leave(), main()])
            button.pack()
        ...
        def compair_answers():
            ...
            if result == 1:
                ...
                lable.pack()
            button = ttk.Button(div, text="Next?", command=lambda: level(hardness, index))
            button.pack()
        ...
        try:
            ...
            entry = ttk.Entry(div)
            entry.pack()
            sumbit = ttk.Button(div, text="Submit Answer", command=compair_answers)
            sumbit.pack()
            div.update()
        except IndexError:
    ...
    def level_select():
        for i, options in enumerate(difficulties):  # i refers to the index
            options = ttk.Button(div, text=options.title(),  command=lambda i=i, hardness=options: level(hardness, i))
            if __debug__:
    ...
    def getusername():
        ...
        def check_name():
            ...
            if username != "":
                ...
                welcoming.destroy()
                level_select()
        welcoming = ttk.Label(root, text=f"Welcome to {__author__.split()[0]}'s Math game!")
        ...
        submit = ttk.Button(root, text="Submit Name", command=lambda: check_name())
